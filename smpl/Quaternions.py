...
class Quaternions:
    ...
    def nearest(self, q, axis=[0, 1, 0]):
        if isinstance(q, Quaternions):
            q = q.qs
        def dot(v1, v2):
            if isinstance(v1, Quaternions):
                v1 = v1.qs
            if isinstance(v2, Quaternions):
                v2 = v2.qs
            return np.sum(v1 * v2, axis=-1, keepdims=True)

        assert q.shape == self.qs.shape
        o_shape = self.qs.shape
        ...
        a = ws * w0 + dot(vs, v0)
        # b = w0*(axis*vs) - ws*(axis*v0) + vs*(axis*v0)
        b = w0*dot(axis, vs) - ws*dot(axis, v0) + dot(vs, np.cross(axis, v0))
        ...
        def getVal(t):
            return self.exp(t*axis/2.0) * self

        r1 = np.reshape(getVal(t1).qs, [-1, 4])
        r2 = np.reshape(getVal(t2).qs, [-1, 4])

        qr1 = np.reshape(dot(q, r1), [-1])
        qr2 = np.reshape(dot(q, r2), [-1])
        
        quats = []
        for _q1, _q2, _r1, _r2 in  zip(qr1, qr2, r1, r2):
            if _q1 > _q2:
                quats.append(_r1)
            else :
                quats.append(_r2)
        tmp = np.array(quats)
        tmp = np.reshape(tmp, o_shape)
        return Quaternions(tmp)

    def to_so3(self, ordering='wxyz'):
        ...
        .. math::
            ...
            \\end{bmatrix}
        """     
        assert self.qs.shape[-1] == 4
        #print(quat.shape[:-1])
        if not np.isclose(np.linalg.norm(self.qs, axis=-1), [1.0]).all():
            raise ValueError("Quaternion must be unit length")

        if ordering is 'xyzw':
            qx, qy, qz, qw = np.split(self.qs, 4, axis=-1)
        elif ordering is 'wxyz':
            qw, qx, qy, qz = np.split(self.qs, 4, axis=-1)
        else:
            raise ValueError(
